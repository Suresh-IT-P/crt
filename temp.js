...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderVehicleOptions(distance, duration = 0) {
        ...
        // Auto-switch to outstation if Local ride is > 100 KM
        if (currentCategory === 'local' && distance > 100) {
            alert('Distance exceeds 100 KM for a local ride. Automatically switching to Outstation.');
            const outstationTab = Array.from(categoryBtns).find(b => b.dataset.category === 'outstation');
            if (outstationTab) outstationTab.click();
            return; // The click will re-trigger calculateFare/renderVehicleOptions
        }
    ...
    // --- ZERO KEY Autocomplete (Nominatim via server proxy) ---
    function setupAutocomplete(inputId, suggestionBoxId) {
        ...
        input.addEventListener('input', () => {
            ...
            timeout = setTimeout(async () => {
                const url = `${API_BASE_URL}/api/proxy/geocode?q=${encodeURIComponent(query)}&limit=6&lang=en`;
                try {
                    ...
                    if (data.features && data.features.length > 0) {
                        ...
                        data.features.forEach(feature => {
                            ...
                            // Build a clean, short, readable label
                            const parts = [];
                            if (p.name) parts.push(p.name);
                            if (p.road && p.road !== p.name) parts.push(p.road);
                            if (p.suburb && p.suburb !== p.name && p.suburb !== p.road) parts.push(p.suburb);
    ...
    window.confirmBookingWithTerms = async function() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Redirect to Active Ride map screen
